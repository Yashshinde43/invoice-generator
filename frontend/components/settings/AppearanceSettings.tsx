      ...
      {themes.map(({ value, label, desc, icon: Icon }) => {
        ...
        return (
          ...
          >
            ...
            <Icon className={`h-5 w-5 mb-2 ${active ? "text-emerald-600 dark:text-emerald-400" : "text-slate-500 dark:text-slate-400"}`} />
            <p className={`text-sm font-semibold mb-0.5 ${active ? "text-emerald-700 dark:text-emerald-300" : "text-slate-800 dark:text-slate-200"}`}>
              {label}
            </p>
